  ...
  const addToStateData = (
    ...
    const addData = function (_state: any[] | any, _item: any): void {
      if (_state instanceof Array) {
        ...
        const filteredState: any[] = _state.filter(c => c instanceof Object);
      ...
      if (_state instanceof Array) {
        const index = _state.findIndex((current: any) => {
          ...
          if (current instanceof Object) {
            return current instanceof Object && current.id == itemId;
    ...
    const base: StoreModule = {
      ...
      actions: {
        async getOne(id: any): Promise<any> {
          try {
            const response = await axios().get(`api/${apiKey}/${id}`);
        ...
        async getAll(params: any, callback?: any): Promise<any> {
          try {
            ...
            if (callback instanceof Function) {
              ...
              console.debug('[Store]: Calling custom callback', response);
              this.data = callback(preparePaginatedResponse(response, false));
            } else {
              this.data = preparePaginatedResponse(response, false);
            }
        ...
        async save(params: any): Promise<any> {
          try {
            ...
            useStore().updateStateData(this.data.data, result, true, true);
            return result;
          } catch (error) {
            resolveError(error);
            return error;
        ...
        async delete(id: any): Promise<any> {
          try {
            const response = await axios().delete(`api/${apiKey}/${id}`);
            const result = prepareResponse(response);
            useStore().removeFromStateData(this.data.data, { id }, true);
            return result;
          } catch (error) {
            resolveError(error);
          }
        ...
        async formConfig(params?: any): Promise<any> {
          try {
            const response = await axios().get(`api/${apiKey}/form-config`, {
              params,
            });
            return prepareResponse(response, false);
          } catch (error) {
